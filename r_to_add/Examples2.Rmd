...
Before starting we need some example data. We can take a look at example1.rmd to see how the *pour_* funcitons work in more detail. For now let us just get the data, take a short look at it and move on.
```{r}
...
###*prepare_*periods
The function `prepare_periods` is used to mark the periods of intered. In hydrologal application within Eruope one might be interested to mark the hydrological years (September to August) as such a period. One can do this by... 
```{r}
...
### preparing as such
At last dont forget that we also have a wrapping function, which can make things more concise and clearer. 
xxx 
