#' Aggregate cosdata bykeys 
#' 
#' Provides a wrapper around `dplyr::group_by` and `dplyr::summarise_at` that 
#' allows for a rowise aggregation of `cosdata` by a set of chosen `key` 
#' columns. 
#' 
...
clump <- function(cosdata, 
  ...
  le_result <- dplyr::group_by_(le_data, .dots = key) %>% 
    dplyr::summarise_at(., .cols = data_names, .funs = .funs)
  return(as.tibble(le_result))
}
...
#' 
#' Emulates the `clump` function, but for the `posix` column from `cosdata`.
#' 
