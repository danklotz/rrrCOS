...
objective funcitons can be provided as a list via `.ofuns`. The pre-set 
...
implies, among other things, that the argument can be provided withouth it 
beeing a character). Here are some examples:
...
**Example 1.** Evaluatig the `judge` function withouth arguments executes 
...
**Example 2.** Plotting the computed objective functions as barplots:
```{r, message=3, purl=FALSE, eval=TRUE}
viscos_example() %>%
  judge(., barplot) %>%
