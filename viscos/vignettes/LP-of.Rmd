...
can be computed through the function `judge_compute`. In order to run the
...
to create plots for the main objective functions: `judge_rasterplot`
and `judge_barplot`. Both functions create a list with 4 `ggplot` figures. Each entry
...
functions. The selection is done via the lazy-evaluated `type` argument. 
