...
Metric play an important role in hydrology. Objective functions, $f_{of}(\cdot)$,
are an important part of the hydrological model calibration.
...
different much ink has been spilled for discussing the different merits and
pitfalls of individual objective functions. Thus, with time many different
ones have been proposed. Some are more useful for model comparison and some
objective functions try to address specific problems of other ones.

## Code
This section defines the code for different objective functions. FORTRAN Routines
are used for most of them. The results are slightly different then the ones
...
The variable $\bar{o}$ represents the average of $o$. The $NSE$ can be seen
as the relational the estimator $s$ and the estimator resulting form the
average of the data. It can can have values between minus infinity and 1;
with 1 being the perfect fit, 0 when the mean of $s$ is as good as the mean of
$o$ and negative values are even worse.
