...
- [Explore periodical objective functions](vignettes/LP)
...
```{r, eval = TRUE, echo=FALSE, purl=FALSE}
  # here we define some data for the demonstration plots
  number_of_data <- 50
  mean <- 0
  sd <- 1
...
```{r, fig.align = "center", eval = TRUE, echo=FALSE, purl=FALSE}
  ...
  for (i in 3:demo_length) {
    name_i <- demo_names[i]
    obj_i <- NSE( obs = demo_data["x"] , sim = demo_data[name_i])
    title_i <- paste("NSE = ", round(obj_i,2), sep = "")
    plot_list[[i-2]] <- ggplot(demo_data) +
      geom_line(aes_string(x = "t", y = x), color = "steelblue") +
      geom_line(aes_string(x = "t", y = name_i), color = "palevioletred") +
      xlab( title_i ) +
      ylab("") +
      theme_minimal() +
...
```{r, fig.align = "center", eval = TRUE, echo=FALSE, purl=FALSE}
  ...
  for (i in 3:demo_length) {
    name_i <- demo_names[i]
    obj_i <- KGE( obs = demo_data["x"] , sim = demo_data[name_i])
...
for (i in 3:demo_length) {
  name_i <- demo_names[i]
  obj_i <- pbias( obs = demo_data["x"] , sim = demo_data[name_i])
