...
'get_main_of'. **Note** that periods must be marked first to run
...
function `get_main_of` can be applied to the prepared data 
series and the results are displayed in the console or can be assigned to a 
variable
```{r, message=3, purl=FALSE, eval=TRUE}
require(visCOS)
require(magrittr)
of_values <- get_main_of(get_runoff_example() %>% mark_periods())
of_values
```
