...
```{r, eval = TRUE, echo=FALSE, purl=FALSE}
  ...
  # since I do not want to many dependencies and need to plot more funcitons into 
...
The Nash-Sutcliffe Criterion $NSE$ is by far the most used efficency criterion 
in hydrology. In the hydrolgoical context $x$ usually represents a set of
runoff-observation and $y$ a set of simluations. The $NSE$ is defined in the 
same way as the general definition of the coefficent of determination $R^2$:
...
resulting form the average of hte data. The following figure illustrates 
...
The Kling-Gupta Efficiency $KGE$ was introduced by Gupta etl al (2009) to 
alleviate some of the shortcommings of the $NSE$. In their paper they argue 
why the $NSE$ tends to overate simuations with small variance 
(note: in the context of the paper $\textrm{simulations} = y$) and 
propose the their efficicency criterion instead. 
...
Where $\alpha = σ_y/σ_x$, i.e.: standard deviation $σ$ of the $y$ dividedby
the $σ$  of $x$ and $\beta = \mu_y / \mu_x$ and $\mu$ is the arithmetic mean. 
...
between $x$ and $y$ devided by the sum of $x$: 
...
$$corr = \frac{cov(x,y)}{σ^2_y*σ^2_x},$$

where $cov(...)$ denotes the covariance and $σ^2$ the variance. The correlation 
coefficent can take on values between -1 and 1. The former corresponds to a 
inverse and the latter to a direct relationship and the closer the values 
is to zero the weaker the implied corelation. The following figure taken 
from the illustartes some examples of $corr$ to give a feeling for its usage: 
...
After the calculation of the `dplyr` is used to seperate the different 
...
Oh yeah. We gonna explore soon TM
```{r}
# Dive With OF ------------------------------------------------------------
#' serve with ofun 
#' 
...
### `explore_ofun`
This function is a subfunction of `explore_basins_with_of` and basically simpler
...
The former is the realtion of the strandard devations and therefore a measure 
