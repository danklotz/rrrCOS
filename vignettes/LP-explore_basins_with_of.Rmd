...
thatlets users explore the (hydro-) graphs for the different basins in 
conjunction witha objective functions for the user-selected time-window. The 
calculation of the objective functions is independed of the code defined in 
the section about `computing_of`, sicne simpler constructions are preferable 
in the case at hand and more information is givne to the user. 

The computations of the app are defined in the `server` part and the 
apperearence in the `ui`. The app itself is depicted in the following figure:
...
The app has actually quite a lot of dependencies. Right now these are: 
 ...
 - `dplyr`, for the `selection` of columns and the `filtering` of rows. 
 - `magrittr`, to make the code more easily readible
 - `xts`, because the `dygraphs` package only works correctly if the data is 
 in `xts` format. 
 - `dygraphs` for the interactive visualisation of the time series
 
To make sure that everything is available at runtime all of the dependencies are
called in the head of the function: 
```{r}
...
to the global environment. This is acutally bad, since the fucntion changes the 
varibales of the global environment. But until now, no better solution could be 
...
Note that this appraoch forces users to enumerate their basins with integers, as 
floating-numbers would be transformed to!
```{r}
...
The major readability problems accour because of the unusual formatting enforced
by shiny. The `reactive({})` marks all the interactive parts of the server. 
The calculations are saved in the output-stucutures, which is handed over to the 
...
`strftime(., format = "%d %b %Y)`, which convertes between character 
...
in `explore_ofun`). The `hr` command block seperates this table from the 
