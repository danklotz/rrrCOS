...
the section about `calculating_of`, since simpler constructions are preferable
...
Currently the shiny app needs some pre-calculations. The first part (I) of the 
pre-calculations includes the defensive function `remove_leading_zeros`, that 
removes the leading zeros from the data.frame `runoff_data`.
For example: If a column would be named `QOBS_00001` it would be transformed to
`QOBS_1`. Afterwards, it is checked if the "cleaned" data (`clean_runoff_data`)
has the needed `viscos_options("name_COSposix")` column. If it does not, then
the column is created with `prepare_complete_date`. 
In the second part (II)
the actual pre-calculations take place. The data is transformed into an `xts`
object (`d_xts`). 

Then (III) the data1 column defined in the `viscos_options` is marked and the 
numeration of the basins is saved within the variable `d_nums`.**Note** that 
both data series have to include a column for every existing basin and the 
basin number after the data name (e.g. QOobs_0001). The former operation is 
achieved by searching for all the columns with starting with 
`viscos_options("name_data1")` . The latter by removing all the non-digits from 
the column-names and searching for the unique integer.

Note that this approach forces users to enumerate their basins with integers,
as floating-numbers would be transformed!
```{r}
  # (I)
...
version of `extract_objective_functions`. `explore_of` is not exported and
