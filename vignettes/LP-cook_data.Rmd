...
The extracted objective fucntions are: 
- Nash-Sutcliffe Criterion `NSE`
- Kling-Gupta Efficiency `KGE`
- Percentage Bias `pBIAS`
- Correlation `CORR_periods`
...
The body of the funciton is lengthly but not to difficult. As a first step, 
the values, which are not within the periods are excluded from the calculation. 
with the 
```{r}
  runoff_data <- runoff_data[runoff_data[[viscos_options()$name_COSperiod]] > 0] 
  dplyr::filter(period > 0)
  number_of_basins <- runoff_data %>% 
    names %>%
    unique %>% 
    tolower %>%
    grepl(viscos_options()$name_data1 , .) %>% 
    sum
  periods_in_data <- runoff_data[[viscos_options()$name_COSperiod]] %>% unique
  number_of_periods <- periods_in_data %>% length
  # 
  obj_fun  <- list()
  tempOBS <- dplyr::select(runoff_data,starts_with(viscos_options()$name_data1)) %>% 
    unname
  tempSIM <- dplyr::select(runoff_data,starts_with(viscos_options()$name_data2)) %>% 
    unname
  ...
  {
    tempOBS <- filter(runoff_data,period == periods_in_data[k]) %>%
      select(.,starts_with(viscos_options()$name_data1)) %>% 
      unname
    tempSIM <- filter(runoff_data,period == periods_in_data[k]) %>% 
