...
with them. `get_runoff_example` is a small wrapper function to get the data from
...
`data_regex`, as well basins where no observations are available.
...
It is actually so pretty simple, but really useful. Since the solution is
...
(see the chapter about defensive code). With that the preparations are finished,
...
of `runoff_data` are obtained with `names` and removing all the digits with
...
period column is a integer vector, which is 1 if the row is within the period
and 0 if it out of the period.
...
in principle also simple as two ranges (`range_1` and `range_2`) can be
constructed and concatenated.
Note that both solution additionally construct the integer-vector
`out_of_period`. The vector contains information about the moths which are not
...
Then, `eval_diff` is used to to mark all the starting months of the runoff data and
save them into an `period` column. The ` pmax(.,0)` operation guarantees that
