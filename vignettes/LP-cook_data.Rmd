...
This chapter defines the basic functions for adapting given (raw) data so 
that it can be used within **visCOS**. In the next chapter we
exemplify the usage of these functions. Afterwards the code of the respective 
functions is given.
...
Usually, one would adapt the ([`viscos_options`](LP-viscos_options.html) to the
given data or transform/name it appropriately. The exemplary data 
(`runoff_example_raw`) does already come with the right naming. Nevertheless, 
here are the available options:
```{r, purl=FALSE, eval=TRUE}
   viscos_options( ) %>% unlist(.)
```

That said, some columns - nameley `QOSI_0001` and `QOSI_0002` - are not needed 
for further analysis. We refer to these data-columns as *junk*. *Junk* are 
either columns in the `data.frame` that are not compatible with the **visCOS** 
functions or column pairs without observations. They might has a purpose for 
some other application or provide valuable feedback for you, but they cannot be 
used by **visCOS**; we need to *throw them away*.

The package provides a function for doing automatically: `remove_junk`. Here
is an example:
```{r, purl=FALSE, eval=TRUE}
  runoff_example_raw %>%
    remove_junk(.) %>%
...
Additionally, the `data.frame` needs to have two different definitions for
the date of a given row (see: [Introduction](introduction.html)). One is based
on an archaic way to write dates, which is often used by (old) models: There are 
for separate columns and each one represents a different time-resolution. These 
are:
...
The other format is a more modern way. That is,
