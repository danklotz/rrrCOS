...
`remove_chunk`(see above) and `prepare.complete_date` is used to ensure that 
...
There is one problem with that solution if `start_month` > `end_month`. Then, 
the last year cannot be extracted with that simply method. Thus, 
`dplyr::mutate` is used to set the out-of-period-dates from the last year to 
zero. 
```{r}
  # (II) mark periods:
...
basin, for the entire time series and for each period sepearately. 
...
(listed within 'Ofun') and the `dplyr` is used to seperate the different 
...
```{r}
  runoff_data %<>%dplyr::filter(period > 0)
  eval_size <- runoff_data %>% 
    names %>%
    unique %>% 
    tolower %>%
    grepl("qobs.*",.) %>% sum
